...
export const WEBINAR_FECHA = '2026-08-18';
export const WEBINAR_HORA = '19:00';

/**
 * Canonical list of mandatory visible registration fields. Every one is mandatory
 * end-to-end: `required` + `aria-required` in markup, presence gate before any
 * submit attempt, and a validation error here. The honeypot is never listed, and
 * neither is the optional Instagram username (see OPTIONAL_REGISTRATION_FIELDS).
 */
...
/**
 * Canonical list of visible-but-optional registration fields. A blank value is
 * accepted (no error, serialized as an empty string); a non-blank value must
 * still pass its format rule, so garbage never reaches the n8n payload.
 */
export const OPTIONAL_REGISTRATION_FIELDS = Object.freeze(['instagramUsername']);
 ...
 * Anything incomplete or differently typed fails closed so neither the redirect
 * nor the server-side Meta CompleteRegistration relay can fire on an
 * unconfirmed GHL write.
 * @param {unknown} data
 * @returns {boolean}
 */
 ...
 * Optional fields (Instagram) are never reported here.
 * @param {unknown} input
...
export function validateRegistration(input) {
  ...
  // Instagram is the only optional field: blank means "not provided" (empty
  // string, no error); a typed value must still be a canonical handle or null.
  const instagramProvided = String(input.instagramUsername ?? '').trim().length > 0;
  const instagramUsername = instagramProvided
  ...
  // Only a typed-but-malformed handle is an error; leaving it empty is allowed.
  if (instagramProvided && !instagramUsername) {
    errors.instagramUsername = 'Ingresa un usuario de Instagram válido o déjalo vacío.';
  }
 ...
 * `instagram_username` is optional: the key is always present and carries an empty
 * string when the visitor left the field blank.
 * @param {{
 *   fullName: string,
 *   email: string,
 *   phone?: string,
...
export function buildRegistrationPayload(input) {
  ...
  const phone = normalizePhone(input.phone) || '';
  // Optional Instagram: canonical `@handle` when valid, otherwise an empty string.
  // Raw/malformed input is never forwarded to n8n, and the root key always exists.
  const canonicalIg = normalizeInstagramUsername(input.instagramUsername) || '';
