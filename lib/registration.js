...
/** Production metrics sink for first-party attribution + registration. */
export const REGISTRATION_ENDPOINT = 'https://metrics.unlockedecom.co/api/attribution-event';
...
/**
 * Exact positive acknowledgement from metrics attribution-event.
 * Requires GHL event-driven write confirmation before success/redirect.
 * @param {unknown} data
 * @returns {boolean}
 */
...
/**
 * Canonical bounded registration payload for metrics attribution-event.
 * event / event_name / surface are fixed library constants (caller overrides ignored).
 * Phone is only emitted as identity.phone; Instagram only as identity.instagram_username
 * (never URL/query/storage/analytics).
 * @param {{
 *   fullName: string,
 *   email: string,
 *   phone?: string,
  ...
  async function submit(payload) {
    ...
    if (cfg.mode === 'webhook') {
      ...
      try {
        ...
        // event / event_name / surface are fixed inside buildRegistrationPayload;
        // cfg.surface and cfg.eventName cannot override the metrics contract.
        // Phone is only identity.phone; Instagram is only identity.instagram_username.
