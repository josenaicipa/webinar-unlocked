...
/**
 * The single explicit Meta event this landing emits for a confirmed registration.
 * Pages also disable Meta automatic event configuration before init, so this is
 * the only registration success event the pixel reports for this surface.
 */
export const META_REGISTRATION_EVENT = 'CompleteRegistration';

/**
 * CompleteRegistration fires only after a confirmed registration success,
 * never on direct thank-you.
 * @param {{ registrationConfirmed?: boolean, isThankYouDirectVisit?: boolean }} input
 */
export function shouldFireMetaRegistrationEvent(input = {}) {
...
/**
 * Stable deduplication id shared with any server-side counterpart.
 * @param {string} eventId
 * @returns {{ eventID: string }}
 */
export function buildMetaEventOptions(eventId) {
...
export function createMetaPixelController(deps = {}) {
   ...
   */
  function trackCompleteRegistration(input = {}) {
    if (
      !shouldFireMetaRegistrationEvent({
    ...
    try {
      fbq('track', META_REGISTRATION_EVENT, {}, buildMetaEventOptions(eventId));
      return true;
    } catch {
      return false;
    }
  ...
  return { trackPageView, trackCompleteRegistration };
}
